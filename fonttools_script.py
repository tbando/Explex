...
def fix_font_tables(style, variant):
    ...
    # OS/2 テーブルを編集
    fix_os2_table(xml, style, flag_hw=True)
    # post テーブルを編集
    fix_post_table(xml, flag_hw=True)
    ...
    # ttLib を使ってさらに細かい修正 (Version, Mac名削除, 正しい名前の設定)
    fix_head_and_name_tables(
        f"{BUILD_FONTS_DIR}/{output_name_base}_os2_post.ttf", style, variant
    )
...
def fix_head_and_name_tables(font_path: str, style: str, variant: str):
    ...
    import re
    v_str = variant
    # ConsoleNF -> Console NF
    v_str = re.sub(r"([a-z0-9])([A-Z])", r"\1 \2", v_str).strip()
    
    family_name = f"{FONT_NAME} {v_str}".strip()
    subfamily_name = style
    if subfamily_name == "BoldItalic":
        subfamily_name = "Bold Italic"
    
    full_name = f"{family_name} {subfamily_name}".strip()
    # PostScript名
    ps_name = f"{FONT_NAME}{variant}-{style}".replace(" ", "-").strip("-")
    
    # Unique ID (Name ID 3) を重複しないように生成
    unique_id = f"{VERSION};{family_name.replace(' ', '')};{ps_name}"

    # メタデータセット (Platform 3, Encoding 1, Language 0x409)
    name_records = {
        0: font["name"].getDebugName(0) or f"Copyright 2026 tbando", # Copyright
        1: family_name,
        2: subfamily_name,
        3: unique_id,
        4: full_name,
        5: f"Version {VERSION}",
        6: ps_name,
        8: "tbando", # Manufacturer
        9: "tbando", # Designer
        10: f"Explex generated by tbando. Components: 0xProto, IBM Plex Sans JP, Nerd Fonts.",
        13: "This Font Software is licensed under the SIL Open Font License, Version 1.1.",
        14: "http://scripts.sil.org/OFL",
    }

    # 3. name テーブルを完全に洗い替える
    name_table = font["name"]
    name_table.names = [] # 一旦全削除 (Platform 1 を確実に消す)

    for name_id, value in name_records.items():
        if value:
            name_table.setName(value, name_id, 3, 1, 0x409)

    # Typographic Family (16, 17) は RIBBI スタイルの場合は不要なので追加しない
    # これにより Font Book での誤認識を防止する

    name_table.names = sorted(name_table.names, key=lambda x: (x.platformID, x.nameID))
    font.save(font_path)
