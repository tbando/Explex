...
AUTHOR_NAME = settings.get("DEFAULT", "AUTHOR_NAME")
FONT_NAME = settings.get("DEFAULT", "FONT_NAME")

# Source versions
PROTO_VER = settings.get("SOURCE_VERSIONS", "0XPROTO")
NERD_VER = settings.get("SOURCE_VERSIONS", "NERDFONTS")
PLEX_VER = settings.get("SOURCE_VERSIONS", "IBMPLEX")
...
def edit_meta_data(font, weight: str, variant: str, cap_height: int, x_height: int):
    ...
    font.copyright = COPYRIGHT.strip()

    # その他のメタデータを設定
    font.appendSFNTName("English (US)", "Designer", AUTHOR_NAME.strip())
    font.appendSFNTName("English (US)", "Manufacturer", AUTHOR_NAME.strip())
    description = f"Explex generated by {AUTHOR_NAME}.\nComponents:\n- 0xProto: v{PROTO_VER}\n- IBM Plex Sans JP: v{PLEX_VER}\n- Nerd Fonts: v{NERD_VER}"
    font.appendSFNTName("English (US)", "Descriptor", description.strip())

    # Version は fonttools で後から整形するため、ここではシンプルにセット
